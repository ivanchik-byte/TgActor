# TgCast — Инструмент для симуляции реалистичных диалогов и тестирования сценариев общения в Telegram
...
5. [Ключевые инженерные задачи (Technical Highlights)](#ключевые-инженерные-задачи-technical-highlights)
6. [Архитектура системы (System Architecture)](#архитектура-системы-system-architecture)
7. [Стек технологий](#стек-технологий)
8. [Безопасность и защита данных](#безопасность-и-защита-данных)
9. [Генерация секретного ключа (Python)](#генерация-секретного-ключа-python)
10. [Установка и запуск на Windows](#установка-и-запуск-на-windows)
11. [Установка и запуск на Linux / VPS](#установка-и-запуск-на-linux--vps)
12. [Варианты подключения домена и HTTPS](#варианты-подключения-домена-и-https)
13. [Конфигурация переменные окружения](#конфигурация-переменные-окружения)
14. [Лицензия и отказ от ответственности](#лицензия-и-отказ-от-ответственности)

---

## О проекте

В отличие от традиционного софта для рассылок, **TgCast** сфокусирован на создании **живой и естественной коммуникации**. Несколько подключенных Telegram-аккаунтов выступают в роли персонажей по вашему сценарию:
...
## Ключевые инженерные задачи (Technical Highlights)

В процессе разработки TgCast были решены следующие сложные инженерные и архитектурные задачи:

1. **Динамическое переназначение ролей (Smart Account Fallback)**:
   - При исполнении сценария аккаунт, которому назначена роль, может оказаться недоступен (cooldown, обрыв связи или блокировка).
   - Алгоритм в `scenario_executor.py` на лету подбирает альтернативный активный аккаунт из пула без аварийной остановки сценария и срывов диалоговой цепочки.

2. **Разрешение тредов обсуждений (Discussion Group Resolution)**:
   - В Telegram комментарии к постам каналов отправляются не в сам канал, а в привязанную супергруппу обсуждений.
   - Реализован механизм автоматического определения привязанной группы (`get_discussion_message`) и связывания корневого ID сообщения для корректного ведения древовидных веток комментариев.

3. **Защитный фоновый вход (Staggered Join)**:
   - Для предотвращения блокировок за аномальную активность аккаунты вступают в новые группы обсуждений не одновременно, а плавно в фоновом режиме с расчетом интервалов и случайных задержек.

4. **Патчинг бинарных конструкторов MTProto**:
   - Реализован патч обработки неизвестных конструкторов Telegram на уровне клиентов Hydrogram (`client.py`), гарантирующий стабильную работу при обновлении схемы протокола.

5. **Реактивная шина событий (Real-Time Event Pipeline)**:
   - Связка Redis Pub/Sub и WebSockets обеспечивает трансляцию входящих личных сообщений и статусных логов мониторинга в интерфейс без опрашивающих запросов (polling).

---

## Архитектура системы (System Architecture)

Схема взаимодействия сервисов, пулов аккаунтов и инфраструктурных компонентов:
...
flowchart TD
    ...
    subgraph Backend["Ядро системы TgCast"]
        CM["Монитор каналов"]
        SE["Исполнитель сценариев"]
        PM["Менеджер пулов"]
        WS["WebSocket брокер инбокса"]
    end
...
Подробное описание архитектурных решений доступно в документе [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md).
...
- **Opentele**: Модуль конвертации `tdata` сессий Telegram в рабочие параметры.
...
**Дисклеймер**: TgCast разработан исключительно в исследовательских целях и для демонстрации в портфолио. Автор не несет ответственности за любое использование данного программного обеспечения, нарушающее Условия использования Telegram (Telegram Terms of Service).
