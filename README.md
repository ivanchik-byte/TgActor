# TgActor: управление Telegram-аккаунтами и диалогами
...
TgActor управляет сеткой Telegram-аккаунтов через веб-панель. Программа разыгрывает ветки диалогов под постами в каналах, ставит реакции, перехватывает свежие публикации и собирает входящие сообщения со всех профилей в единый инбокс.
...
- **Режим первого комментария (Channel Sniper / «Первонах»)**:
  - Мгновенный снайпинг новых публикаций в целевых каналах (пауза 0-2 сек).
  - Публикация комментария **от имени вашего Telegram-канала** (`Send as Channel`) с аватаром и названием канала для привлечения подписчиков, либо от личного профиля.
  - Автоматическая подгрузка списка каналов выбранного аккаунта (где он является создателем или администратором).
  - Встроенный фильтр рекламы (`Anti-Ad Shield`): автопропуск спонсорских постов с `erid`, `#реклама` и партнерскими ссылками.
  - AI Prompt Studio: чипы пресетов («Экспертный тезис», «Вопрос по теме», «Практический опыт», «Ирония / реакция») по строгим правилам Humanizer (1 предложение, без эмодзи, без финальной точки).
- **Конструктор сценариев и веток диалогов**: создание цепочек сообщений любой длины с ответами ботов друг на друга, рандомизацией пауз и медиавложениями.
...
- **Мониторинг каналов (Радар)**: программа следит за каналами и, как только выходит новый пост, автоматически отправляет ботов комментировать его по сценарию или в режиме снайпинга.
...
- **Дизайн в стиле Linear / Raycast**: современный темный интерфейс без навязчивых эмодзи, со строгими статусами и мгновенным откликом.
...
![Конструктор диалогов](docs/images/02_scenarios_constructor.png)
...
![Входящие сообщения](docs/images/05_unified_inbox.png)
...
![Управление аккаунтами](docs/images/08_accounts_management.png)
...
![Пример диалога](docs/images/14_telegram_discussion_result.png)
...
4. Откройте файл `.env` в Блокноте и заполните:
   - `SECRET_KEY` и `ENCRYPTION_KEY`: сгенерированный ключ.
   - `ADMIN_PASSWORD`: ваш пароль для входа.
...
Если вы запускаете проект напрямую через Python и Node.js:
