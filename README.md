...
| `DATABASE_URL` | Подключение к СУБД PostgreSQL или SQLite | `postgresql+asyncpg://tgcast:tgcast_password@db:5432/tgcast_db` |
| `REDIS_URL` | Подключение к брокеру сообщений Redis Pub/Sub | `redis://redis:6379/0` |
| `HTTP_PORT` | Внешний порт веб-панели для Caddy прокси | `8080` |
| `DOMAIN` | Ваш домен для авто-выпуска SSL в Caddy | `localhost` |
| `ENABLE_CHANNEL_MONITOR` | Включить автоматический фоновый монитор каналов | `False` |
| `ENABLE_INBOX_LISTENER` | Включить автоматический фоновый слушатель инбокса | `False` |

---

## Управление фоновыми службами (Feature Toggles)

Для предотвращения незапланированных нагрузок и поочередного включения нужных модулей, система поддерживает гибкое опциональное управление фоновыми процессами.

Вы можете включать и отключать фоновые компоненты через переменные в файле `.env`:

1. **Фоновый мониторинг каналов (`ENABLE_CHANNEL_MONITOR`)**:
   - **`False` (по умолчанию)**: Монитор каналов отключен. Вы можете запускать сценарии вручную по ссылке на пост или в симуляторе.
   - **`True`**: Монитор опрашивает отслеживаемые каналы каждые 30 секунд и автоматически запускает сценарии при появлении новых публикаций.

2. **Слушатель личных сообщений (`ENABLE_INBOX_LISTENER`)**:
   - **`False` (по умолчанию)**: Инбокс не опрашивает входящие личные сообщения в фоновом режиме.
   - **`True`**: Подключенные аккаунты постоянно слушают входящие сообщения и мгновенно пересылают их в централизованный инбокс по WebSockets.
