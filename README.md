...
- **Детализированный журнал активности (Action Logs v2.1)**: Трекинг всех событий ботов на сервере в реальном времени с 3 режимами вывода (Хронология, Live-консоль, Аналитика), фильтрацией по аккаунтам/периоду/статусам, инспектором JSON-пейлоадов и адаптацией под темы.
...
Вы можете подключить собственный домен для доступа к веб-панели одним из двух способов:

### Вариант 1. Через сервисы проксирования (Cloudflare)
1. Добавьте ваш домен в бесплатный тариф **Cloudflare** и направите A-запись на IP-адрес вашего сервера.
...
### Вариант 2. Стандартный запуск без домена (HTTP / IP-адрес)
...
Ниже приведены основные переменные окружения из файла `.env`:

| Переменная | Описание | Значение по умолчанию |
| :--- | :--- | :--- |
| `ADMIN_PASSWORD` | Пароль для входа в веб-панель управления | `admin` |
| `SECRET_KEY` | Секретный ключ для Fernet-шифрования Telegram-сессий | `generate_random_secret_key` |
| `DATABASE_URL` | Подключение к СУБД PostgreSQL или SQLite | `postgresql+asyncpg://tgactor:tgactor_password@db:5432/tgactor_db` |
| `REDIS_URL` | Подключение к брокеру сообщений Redis Pub/Sub | `redis://redis:6379/0` |
| `ENABLE_CHANNEL_MONITOR` | Включить автоматический фоновый монитор каналов | `True` |
| `ENABLE_INBOX_LISTENER` | Включить автоматический фоновый слушатель инбокса | `True` |

---

## Управление фоновыми службами (Feature Toggles)

По умолчанию фоновые сервисы включены в автоматическом режиме:

1. **Фоновый мониторинг каналов (`ENABLE_CHANNEL_MONITOR`)**:
   - **`True` (по умолчанию)**: Монитор опрашивает отслеживаемые каналы каждые **10 секунд** и автоматически запускает сценарии при появлении новых публикаций.
   - **`False`**: Монитор каналов отключен. Запуск сценариев доступен вручную по ссылке на пост или в симуляторе.

2. **Слушатель личных сообщений (`ENABLE_INBOX_LISTENER`)**:
   - **`True` (по умолчанию)**: Подключенные аккаунты постоянно слушают входящие сообщения и мгновенно пересылают их в централизованный инбокс по WebSockets.
   - **`False`**: Отключает фоновое слушание входящих сообщений.
...
**Дисклеймер**: TgActor разработан исключительно в исследовательских целях и для дополнения в портфолио. Автор не несет ответственности за любое использование данного программного обеспечения, нарушающее Условия использования Telegram (Telegram Terms of Service).
