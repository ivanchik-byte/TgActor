...
flowchart TD
    ...
    subgraph Backend["Ядро системы TgActor"]
        ...
        SE["Исполнитель сценариев<br/>(scenario_service.py)"]
        PM["Менеджер пулов<br/>(pool_service.py)"]
...
3. **Распределение аккаунтов из пулов (Account Pools Allocation)**:
   - Исполнитель запросов (`scenario_service.py`) запрашивает активные аккаунты из пула комментирования и пула реакций.
...
## 5. Записи архитектурных решений (ADR)
...
- **Контекст**: Одновременное вступление нескольких аккаунтов в новую группу обсуждения приводит к срабатыванию спам-фильтров Telegram.
...
- **Решение**: Реализована шина сообщений Redis Pub/Sub. Слушатели сессий публикуют входящие события в Redis, а модуль `inbox_ws.py` транслирует их по WebSocket в клиентский интерфейс React.
