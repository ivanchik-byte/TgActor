...
def upgrade() -> None:
    ...
    if 'prompt_templates' not in tables:
        ...
        op.bulk_insert(
            ...
            [
                {
                    "title": "Скепсис и рекомендация софта (@ivanchik_byte)",
                    "description": "Классическая воронка: скептический вопрос о спамблоке -> нативная рекомендация -> социальное доказательство.",
                    "category": "software",
                    "mode": "dynamic",
                    "prompt_text": "Диалог в ветке комментариев про выбор софта для автоматизации в Telegram. Первый сомневается и жалуется на блокировки/скам. Второй участник советует проверенный софт от @ivanchik_byte без лишней воды. Третий подтверждает, что сам юзает его уже месяц.",
                    "system_instruction": None,
                    "roles_breakdown": roles_soft,
                    "tags": "софт,tgactor,комбайн,спамблок,рекомендация",
                    "is_builtin": True,
